"""Tests for Layer 1: Sector-level disk access."""
...
class TestSequentialDSDSectorImage:
    ...
    def test_physical_offset_is_sequential(self):
        ...
        buffer = bytearray(204800)  # 800 sectors
        dsd = SequentialDSDSectorImage(buffer)
...
class TestInterleavedDSDSectorImage:
    ...
    def test_num_sectors_40_track(self):
        ...
        buffer = bytearray(204800)
        dsd = InterleavedDSDSectorImage(buffer, tracks_per_side=40)
        assert dsd.num_sectors() == 800

    def test_num_sectors_80_track(self):
        """80-track interleaved DSD has 1600 sectors."""
        buffer = bytearray(409600)
        dsd = InterleavedDSDSectorImage(buffer, tracks_per_side=80)
        assert dsd.num_sectors() == 1600
...
class TestDSDSideSectorImage:
    ...
    def test_constructor_validates_side(self):
        ...
        buffer = bytearray(204800)
        interleaved = InterleavedDSDSectorImage(buffer, tracks_per_side=40)
    ...
    def test_80T_side0_sector_range(self):
        ...
        buffer = bytearray(409600)  # 80T DSD
        interleaved = InterleavedDSDSectorImage(buffer, tracks_per_side=80)
...
class TestSectorImageInterface:
    ...
    def sector_image(self, request):
        ...
        buffer = bytearray(204800)  # Large enough for all types
        if request.param == "ssd":
            return SSDSectorImage(buffer)
        elif request.param == "sequential_dsd":
            return SequentialDSDSectorImage(buffer)
